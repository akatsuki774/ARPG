...
namespace DTL.Shape {
    ...
    public sealed class RogueLike : DTL.Range.RectBaseRogueLike<RogueLike>, IDrawer<int> {
        ...
        private List<RogueLikeOutputNumber> roomList;
        ...
        public List<MatrixRange> GetRoomList() {
            List<MatrixRange> destList = new List<MatrixRange>();
            foreach( var room in roomList ) {
                MatrixRange matrix = new MatrixRange( room.x, room.y, room.w, room.h );
                destList.Add( matrix );
            }
            return destList;
        }
        ...
        bool DrawNormal(int[,] matrix) {
            ...
            roomList = new List<RogueLikeOutputNumber>();
            var branchPoint = new List<RogueLikeOutputNumber>();
            var isWay = new List<bool>();

            // 最初の部屋を生成
            if (!MakeRoom(matrix, sizeX, sizeY, roomList, branchPoint, isWay, (int)sizeX / 2, (int)sizeY / 2,
                (Direction)rand.Next(directionCount), true)) return false;

            // 機能配置
            for (uint i = 1; i < maxWay; ++i) {
                if (!CreateNext2(matrix, sizeX, sizeY, roomList, branchPoint, isWay)) break;
            }
        ...
        private bool PlaceOutputNumber(int[,] matrix, uint sizeX, uint sizeY, RogueLikeOutputNumber rect, int tile) {
            ...
            //                return false;
            //            }
            //            if ( matrix[startY + y, startX + x] == rogueLikeList.insideWallId ) {
            //                insideCount++;
            //                if ( insideCount >= 3 ) {
            //                    return false;
            //                }
            ...
            //                if ( matrix[y, x] != rogueLikeList.wayId ) {
            //                    matrix[y, x] = rogueLikeList.insideWallId;
            //                } else {
            //                    matrix[y, x] = rogueLikeList.wayId;
            //                }
            //            } else {
            //                matrix[y, x] = tile;
            //            }
